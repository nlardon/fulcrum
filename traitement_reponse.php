...
function connect_db($servername, $username, $password, $dbname) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while($row = mysqli_fetch_assoc($result_q)) {
            ...
            $ans[1] = $row["1_question"];
            $ans[2] = $row["2_question"];
            $ans[3] = $row["3_question"];
            $ans[4] = $row["4_question"];
            $ans_correct = $row["correct_question"];
            if($ans_correct == 1) $ans_correct_text = $ans[1];
            elseif($ans_correct == 2) $ans_correct_text = $ans[2];
            elseif($ans_correct == 3) $ans_correct_text = $ans[3];
            else $ans_correct_text = $ans[4];
        }        
    } else {
        echo "0 results";
    }
    ...
    $result_no_quiz = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result_no_quiz) > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result_no_quiz)) {
    ...
    <div class="w-full max-w-6xl container-card bg-white rounded-xl p-6 sm:p-8">
        ...
        <div class="mb-10 p-5 bg-gray-50 rounded-xl border border-gray-200">
            ...
            <div class="flex items-end justify-around h-64 border-b border-gray-300 pb-2">
                ...
                $chartData = [
                    '1' => ['label' => $ans[1], 'color' => 'bg-blue-500', 'text' => 'text-blue-700'],
                    '2' => ['label' => $ans[2], 'color' => 'bg-green-500', 'text' => 'text-green-700'],
                    '3' => ['label' => $ans[3], 'color' => 'bg-yellow-400', 'text' => 'text-yellow-700'],
                    '4' => ['label' => $ans[4], 'color' => 'bg-red-500', 'text' => 'text-red-700'],
                ];
                ...
                foreach ($chartData as $key => $data) {
                    ...
                    // Libellé
                    echo '<div class="text-sm font-medium mt-3 whitespace-nowrap text-center ' . $data['text'] . '">' . $ans[$key] . '</div>';
                    echo '</div>';
                }
                ?>
            </div>
